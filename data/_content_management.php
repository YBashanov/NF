...
//$data['scrubs'] = $content->scrubs($a_menu, $page);
//include "{$separator}modules/svg/clock2.php";
...
if (
	...
	//if ($page == "otzivi") include "{$separator}templates/{$global['template']}/html/modules/config.php";
	//include "{$separator}templates/{$global['template']}/modules/m_hmenu/config.php";
    includes ("{$separator}templates/{$global['template']}/_modules_cont_management.php");
	
	
	include "{$separator}templates/{$global['template']}/html/config.php";
	include "{$separator}templates/{$global['template']}/style/config.php";
	
	//system type - тип страницы определен системой
	if ($merger['page_type'] == 0) {
		if (@file_exists("{$separator}templates/{$global['template']}/html/t_{$page}/config.php")) {
			include "{$separator}templates/{$global['template']}/html/t_{$page}/config.php";
		}
		else {
			if ($global['createConfigs']) {
				...
				if ($file->catalogCreate("{$separator}templates/{$global['template']}/html/t_{$page}")){
					if ($errorTrue) $wrap->add(2, "Каталог template/../{$page}/ создан", "_content_management");
				}
				if ($file->createFileFromTemplate("{$separator}templates/{$global['template']}/html/t_{$page}/config.php", $page, "config")) {
					if ($errorTrue) $wrap->add(2, "Файл {$page}/config создан", "_content_management");
				}
				if ($file->createFileFromTemplate("{$separator}templates/{$global['template']}/html/t_{$page}/t_content.html", $page, "t_content")) {
...
elseif (
	...
	if (@file_exists("{$separator}templates/{$global['template']}/html/t_auxil/config.php")) {
		include "{$separator}templates/{$global['template']}/html/t_auxil/config.php";
	}
