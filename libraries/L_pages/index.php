...
/*
26.04.12 - класс, работающий со страницами
*/
...
class L_pages{
	...
	public static function init($error){
		if ( self::$thisObject == null ){
			self::$thisObject = new L_pages();
			self::$error = $error;
			return self::$thisObject;
		}
		else {
			echo "L_pages: Объект L_pages уже создан ранее!";
			self::$error->add(2, "Ошибка. Один объект уже существует", 'L_pages');
			exit();
		}
	}
...
*/
	public function num_pages ($count, $countInPage = 25, $nowPage = 1, $class = "", $pathToPage = "", $getLink = "?", $action = "anchor", $function = "func(", $pageNPAGE = "npage", $pageINPage="inPage", $totalAnchors=0) {
		...
		if (! is_int($countInPage) || $countInPage == 0){
			self::$error->add(2, "Ошибка. Второй параметр не число (или =0)", 'L_pages');
			return false;
		}
