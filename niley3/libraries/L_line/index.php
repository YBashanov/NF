...
class L_line{
	...
	public static function init($pagePrefix = "", $error = ""){
		if ( self::$thisObject == null ){
			self::$error 		= $error;
			self::$pagePrefix 	= $pagePrefix;
			self::$thisObject = new L_line();

			if (self::$progressive == false) self::$thisObject->created_line();
			//prog_created_line - вызывается из includes
			
			return self::$thisObject;
		}
		else {
			echo "L_line: Объект L_line уже создан ранее!";
			self::$error->add(2, "Ошибка. Один объект уже существует", 'L_line');
			exit();
		}
	}
	...
	//(присвоение реализуется в includes)
...
class Line_inquiryFast{
	...
	public static function init($error){
		if ( self::$thisObject == null ){
			self::$error = $error;
			self::$thisObject = new Line_inquiryFast();
			return self::$thisObject;
		}
		else {
			echo "Объект Line_inquiryFast уже создан ранее!";
			self::$error->add(2, "Ошибка. Один объект уже существует", 'Line_inquiryFast');
			exit();
		}
	}
	
	//создание линии
	public function created_line($line) {
		$this->preg_line = $line;
		return $this->processing_line();
	}
	
	//дополнительная функция для обратной совместимости (от 2.41 + ниже)
	public function getArray($line){
		return $this->created_line($line);
	}
	...
	private function processing_line(){
		if (self::$progressive == false) {
			...
			$question_mark = explode("?", $this->preg_line);
			
			if (count($question_mark) == 2) {
				$array_GET = explode("&", $question_mark[1]);
				$array = array();
				for ($i = 0; $i<count($array_GET); $i++) {
					$keyANDval = explode("=", $array_GET[$i]);
					$array[$keyANDval[0]] = $keyANDval[1];
	...
	//установить вручную тип использования функционала для подготовки url
	//type=true - отделяет фрейм-часть /// от get-массива
	//type=false- сразу разбивает get-массив, подразумевается, что исходная url не содержит в себе фрейм-части
	public function setTypeLine ($type = true) {
		self::$progressive = $type;
	}
}
?>