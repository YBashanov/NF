...
//создание таблицы в базе
	private function create($query, $link){
		if ( $this->query($query, $link, 'create') ) { return TRUE;}
			...
			$this->error->add(1, "Не удалось создать таблицу. Ошибка: ".mysqli_error()."<br />Запрос: {$query}", get_class($this));
			return FALSE;
		}
	}
	...
	private function pconnect(){
		if (!$this->link){
			...
			if ( !$link ) {
				...
				$this->error->add(1, "Pconnect: Невозможно подключиться к серверу. Ошибка: ".mysqli_error(), get_class($this));
				return;
			}
			...
			if ( @mysqli_select_db($link, $db)){}
				...
				$this->error->add(1, "Pconnect: Невозможно открыть БД {$db}. Ошибка: ".mysqli_error(), get_class($this));
				return;
			}
			...
			if (@mysqli_query($link, "SET CHARACTER SET ".$this->character_set)){}
				...
				$this->error->add(1, "Pconnect: Невозможно перевести кодировку. Ошибка: "
...
//создание соединения с базами с разрывом
	private function connect(){
    	...
    	if ( !$link ) {
    		...
    		$this->error->add(1, "Connect: Невозможно подключиться к серверу. Ошибка: ".mysqli_error(), get_class($this));
    	}
		
    	if ( @mysqli_select_db($link, $db)){}
		else {
			echo "Connect: Невозможно открыть БД.";
			$this->error->add(1, "Connect: Невозможно открыть БД {$db}. Ошибка: ".mysqli_error(), get_class($this));
			return;
		}
		...
		if (@mysqli_query($link, "SET CHARACTER SET ".$this->character_set."")){}
			...
			$this->error->add(1, "Connect: Невозможно перевести кодировку. Ошибка: "
				.mysql_error().", кодировка - ".$this->character_set, get_class($this));
			return;
		}
	...
	private function close_connect( $link ){
		...
		if (is_resource($link)){
			...
			if ( @mysqli_close ( $link )) { return true; } 
				...
				$this->error->add(1, "Close_connect: Невозможно отключиться от сервера. Ошибка: ".mysqli_error(), get_class($this));
				return false;
			}
		}
		else{
...
//выбрать всё - возвращает полный двумерный массив таблицы, первой мерой которого является 
	...
	public function select($table, $where = 'true', $what='*', $key = 'id', $classWhichCalled='', $mysql_num = false) {
		...
		else $this->error->add(1, 'оператор SELECT: '.mysqli_error().'<Br />запрос: '.$query, $classWhichCalled);
		return $result;
	}
...
//выбрать только одну строку - 
	...
	public function select_line($table, $where = 'true', $what='*', $classWhichCalled=''){
		...
		$res = $this->query ( $query, $link, 'select_line' ) or (
				$this->error->add(1, 'select_line: '.mysqli_error().'<Br />запрос: '.$query, $classWhichCalled)
			);
		//if ($res)...
...
//выбрать только один столбец, 
	...
	public function select_column($table, $where = 'true', $what='*', $q_array='id', $classWhichCalled=''){
		...
		$res = $this->query ( $query, $link, 'select_column' ) or (
				$this->error->add(1, 'select_column: '.mysqli_error().'<Br />запрос: '.$query, $classWhichCalled)
			);
...
//добавить новую строку в базу
	...
	public function insert($table, $data, $classWhichCalled=''){
		...
		if(!$this->query($query,'','insert')){
		 	$this->error->add(1, 'оператор INSERT: '.mysqli_error().'<Br />запрос: '.$query, $classWhichCalled);
			return false;
		}
		return true;
	}
...
//обновление данных в базе
	public function update($table, $data, $where, $classWhichCalled='' ){
		...
		if ( ! $this->query($query) ){
			$this->error->add(1, 'оператор UPDATE: '.mysqli_error().', query: '.$query, $classWhichCalled);
			return false;
		}
		return true;
	}
