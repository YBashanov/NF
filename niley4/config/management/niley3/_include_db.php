...
if (
	...
	include "{$separator}php/data/{$global['style_data']}/d_common.php";
}
...
if (
	...
	if ($global['server'] == "internet") {
		if (@file_exists("{$separator}php/modules/{$global['style_modules']}/counter_yandexmetrika.php")) {
			include "{$separator}php/modules/{$global['style_modules']}/counter_yandexmetrika.php";
		}
	}
	
	
	if ($merger['page_type'] == 0) {
		if (@file_exists("{$separator}php/data/{$global['style_data']}/d_{$page}.php")) {
			include "{$separator}php/data/{$global['style_data']}/d_{$page}.php";
		}
		else {
			if ($global['createConfigs']) {
				if ($errorTrue) $wrap->add(2, "Для данной страницы нет файла выдачи данных d_{$page}/", "_include_db");
				if ($file->createFileFromTemplate("{$separator}data/{$global['style_data']}/d_{$page}.php", $page, "d_page")) {
	...
	//тип страницы не определен (либо определен в админке)
		...
		$what = "id, type, path";
		$_include_db_templates = $db->select_line($table, $where, $what, "id", __FILE__);
